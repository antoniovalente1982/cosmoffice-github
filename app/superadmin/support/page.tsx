            ...
            <div className="max-w-5xl mx-auto">
                ...
                <div className="flex items-center justify-between mb-8">
                    <div>
                        ...
                        <p className="text-sm text-slate-500 mt-1">Gestisci le richieste di assistenza dei clienti</p>
                    </div>
                        ...
                        {filtered.map(t => {
                            ...
                            return (
                                <div key={t.id}
                                    ...
                                    {/* Ticket Header */}
                                    <div
                                        className="p-5 cursor-pointer hover:bg-white/[0.02] transition-colors rounded-2xl"
                                        ...
                                        <div className="flex items-start justify-between gap-4">
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <div className="flex items-center gap-4 text-[11px] text-slate-400 flex-wrap">
                                                    ...
                                                    {t.requester_role && (
                                                        <span className="px-1.5 py-0.5 rounded text-[9px] font-bold uppercase bg-white/5">
                                                            {t.requester_role}
                                                        </span>
                                                    )}
                                                </div>
                                            ...
                                            <div className="flex items-center gap-2 shrink-0">
                                                <span className="text-[10px] text-slate-600">
                                                    {new Date(t.created_at).toLocaleDateString('it-IT', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' })}
                                                </span>
                                    ...
                                    {isExpanded && (
                                        <div className="px-5 pb-5 border-t border-white/5 pt-4 space-y-4">
                                            ...
                                            {/* Admin Notes */}
                                            <div>
                                                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1 block">Note Admin</label>
                                                <textarea
                                            ...
                                            {(t.status === 'in_progress' || t.status === 'open') && (
                                                <div>
                                                    <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1 block">Risoluzione</label>
                                                    <textarea
                                                        value={resolutionInputs[t.id] ?? t.resolution ?? ''}
                                                        onChange={(e) => setResolutionInputs(p => ({ ...p, [t.id]: e.target.value }))}
                                                        placeholder="Descrivi come è stato risolto..."
                                                        rows={2}
                                                        className="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-xl text-xs text-slate-200 placeholder:text-slate-600 focus:outline-none focus:border-emerald-500/30 resize-none"
                                                    />
                                                </div>
                                            )}
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                {t.status === 'open' && (
                                                    <button
                                                        onClick={() => updateTicket(t.id, {
                                                            status: 'in_progress',
                                                            admin_notes: noteInputs[t.id] ?? t.admin_notes,
                                                        })}
                                                ...
                                                {(t.status === 'open' || t.status === 'in_progress') && (
                                                    <button
                                                        onClick={() => updateTicket(t.id, {
                                                            status: 'resolved',
                                                            admin_notes: noteInputs[t.id] ?? t.admin_notes,
                                                            resolution: resolutionInputs[t.id] ?? t.resolution,
                                                        })}
                                                        disabled={processing === t.id}
                                                ...
                                                {/* Save notes button */}
                                                {(noteInputs[t.id] !== undefined || resolutionInputs[t.id] !== undefined) && (
                                                    <button
                                                        onClick={() => updateTicket(t.id, {
                                                            admin_notes: noteInputs[t.id] ?? t.admin_notes,
                                                            resolution: resolutionInputs[t.id] ?? t.resolution,
