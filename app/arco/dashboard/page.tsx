...
function countField(responses: any[], field: string): Record<string, number> {
  const counts: Record<string, number> = {};
  responses.forEach(r => { const v = r[field]; if (v) counts[v] = (counts[v] || 0) + 1; });
  return counts;
}

function avgField(responses: any[], field: string): number {
      ...
      {entries.map(([key, count]) => (
        <div className="arco-bar-row" key={key}>
          ...
          <div className="arco-bar-track">
            <div className="arco-bar-fill" style={{ width: `${Math.max((count / total) * 100, 8)}%` }}>
...
function ToolsChart({ responses }: { responses: any[] }) {
  ...
  responses.forEach(r => {
    (r.ai_tools_used || []).forEach((t: string) => { toolCounts[t] = (toolCounts[t] || 0) + 1; });
  });
  const total = responses.length || 1;
  return <BarChart data={toolCounts} total={total} />;
}

function FreeTextList({ responses, field, label }: { responses: any[]; field: string; label: string }) {
  const texts = responses.map(r => ({ text: r[field], name: r.participant?.first_name + ' ' + r.participant?.last_name, dept: r.participant?.department })).filter(t => t.text?.trim());
  if (!texts.length) return <p style={{ color: 'var(--arco-text-dim)', fontSize: '0.9rem' }}>Nessuna risposta</p>;
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
      {texts.map((t, i) => (
        <div key={i} style={{ padding: '0.75rem 1rem', background: 'var(--arco-bg-input)', borderRadius: '10px', border: '1px solid var(--arco-border)' }}>
          <p style={{ fontSize: '0.9rem', lineHeight: 1.5, marginBottom: '0.25rem' }}>{t.text}</p>
          <p style={{ fontSize: '0.75rem', color: 'var(--arco-text-dim)' }}>— {t.name} ({t.dept})</p>
        </div>
      ))}
    </div>
  );
...
export default function DashboardPage() {
  ...
  const TABS: { key: TabKey; label: string }[] = [
    ...
    { key: 'freetext', label: '💬 Risposte libere' },
  ...
  if (loading) return (
    <>
      ...
      <div className="arco-container wide"><div className="arco-spinner" /></div>
    </>
  );
      ...
      <div className="arco-container wide">
        ...
        <div className="arco-dash-grid arco-animate-in">
          ...
          <div className="arco-stat-card">
            ...
            <div className="arco-stat-sub">{rate}% completion rate</div>
          </div>
          <div className="arco-stat-card">
            <div className="arco-stat-label">Apertura al cambiamento</div>
            <div className="arco-stat-value">{avgField(responses, 'change_openness')}</div>
            <div className="arco-stat-sub">media su 10</div>
          </div>
          <div className="arco-stat-card">
            <div className="arco-stat-label">Entusiasmo workshop</div>
          ...
          <div className="arco-animate-in">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
              <div className="arco-stat-card">
                ...
                <div className="arco-stat-value">{responses.filter(r => r.knows_vibe_coding).length}</div>
                <div className="arco-stat-sub">su {responses.length}</div>
              </div>
              <div className="arco-stat-card">
                <div className="arco-stat-label">Usa AI per lavoro</div>
                <div className="arco-stat-value">{responses.filter(r => r.ai_work_usage).length}</div>
                <div className="arco-stat-sub">su {responses.length}</div>
              </div>
              <div className="arco-stat-card">
                <div className="arco-stat-label">Vuole creare tool</div>
                <div className="arco-stat-value">{responses.filter(r => r.interested_in_building_tools).length}</div>
                <div className="arco-stat-sub">interessati al no-code AI</div>
              </div>
              <div className="arco-stat-card">
                <div className="arco-stat-label">Tempo medio compilazione</div>
                <div className="arco-stat-value">{avgField(responses, 'completion_time_seconds') ? Math.round(avgField(responses, 'completion_time_seconds') / 60) + 'min' : '—'}</div>
              </div>
            </div>
          </div>
        )}
        ...
        {tab === 'change' && (
          <div className="arco-animate-in">
            ...
            <div className="arco-section">
              <h3 className="arco-section-title">Esperienze passate con il cambiamento</h3>
            ...
            <div className="arco-section">
              ...
              <FreeTextList responses={responses} field="change_biggest_fear" label="Paura" />
            </div>
          </div>
        )}
        ...
        {tab === 'ai' && (
          <div className="arco-animate-in">
            ...
            <div className="arco-section">
              <h3 className="arco-section-title">Frequenza utilizzo AI</h3>
        ...
        {tab === 'freetext' && (
          <div className="arco-animate-in">
            <div className="arco-section">
              ...
              <FreeTextList responses={responses} field="what_would_automate" label="" />
            </div>
            <div className="arco-section">
              <h3 className="arco-section-title">⏰ Attività che rubano più tempo</h3>
              <FreeTextList responses={responses} field="biggest_time_waster" label="" />
            </div>
            <div className="arco-section">
              <h3 className="arco-section-title">🦸 Superpotere desiderato dall&apos;AI</h3>
              <FreeTextList responses={responses} field="dream_superpower" label="" />
            </div>
            <div className="arco-section">
              <h3 className="arco-section-title">❓ Domande per Antonio</h3>
              <FreeTextList responses={responses} field="specific_questions" label="" />
            </div>
          </div>
        )}
        ...
        {tab === 'participants' && (
          <div className="arco-section arco-animate-in">
            ...
            <div style={{ marginTop: '1rem' }}>
              ...
              {!participants.length && <p style={{ color: 'var(--arco-text-dim)', textAlign: 'center', padding: '2rem' }}>Nessun partecipante ancora registrato</p>}
            </div>
          </div>
        )}
      </div>
