            ...
            <div className="flex items-center gap-3 flex-wrap">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button
                        ...
                        className="px-4 py-2 bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-400 hover:to-orange-400 text-white text-sm font-bold rounded-xl flex items-center gap-2 transition-all shadow-lg shadow-amber-500/20"
                    >
                        <KeyRound className="w-4 h-4" />
                        Link Owner
                    </button>

                    {/* Filtri & Ordina toggle */}
                    <div className="relative">
                        <button
                            onClick={() => setShowFiltersPanel(!showFiltersPanel)}
                            className={`px-3 py-2 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all border ${showFiltersPanel || planFilter || statusFilter || sortMode !== 'name_asc'
                        ...
                        >
                            ...
                            Filtri
                            {(planFilter || statusFilter || sortMode !== 'name_asc') && (
                                <span className="w-1.5 h-1.5 rounded-full bg-cyan-400" />
                            )}
                        </button>

                        {showFiltersPanel && (
                            <div className="absolute right-0 top-full mt-2 w-72 rounded-xl border border-white/10 p-4 space-y-4 z-50 shadow-2xl" style={{ background: '#0f172a' }}>
                                {/* Piano */}
                                <div>
                                    <p className="text-[10px] text-slate-500 uppercase font-bold tracking-wider mb-2">Piano</p>
                                    <div className="flex gap-1.5">
                                        {['', 'premium', 'demo'].map(p => (
                                            <button key={p} onClick={() => { setPlanFilter(p); setPage(1); }}
                                                className={`flex-1 px-2 py-1.5 rounded-lg text-[11px] font-semibold transition-all border ${planFilter === p
                                ...
                                {/* Stato */}
                                <div>
                                    <p className="text-[10px] text-slate-500 uppercase font-bold tracking-wider mb-2">Stato</p>
                                    <div className="flex gap-1.5">
                                        {[
                                            { value: '', label: 'Tutti' },
                                            ...
                                            <button key={s.value} onClick={() => { setStatusFilter(s.value); setPage(1); }}
                                                className={`flex-1 px-2 py-1.5 rounded-lg text-[11px] font-semibold transition-all border ${statusFilter === s.value
                                                    ? 'bg-cyan-500/20 text-cyan-300 border-cyan-500/30'
                                                    : 'bg-black/30 text-slate-400 border-white/5 hover:bg-white/5'
                                                    }`}>
                                                {s.label}
                                ...
                                {/* Ordina */}
                                <div>
                                    <p className="text-[10px] text-slate-500 uppercase font-bold tracking-wider mb-2">Ordina per</p>
                                    <div className="flex gap-1.5 flex-wrap">
                                        <button onClick={() => setSortMode(sortMode === 'name_asc' ? 'name_desc' : 'name_asc')}
                                            className={`px-2.5 py-1.5 rounded-lg text-[11px] font-semibold transition-all border ${sortMode.startsWith('name_')
                                        ...
                                        <button onClick={() => setSortMode(sortMode === 'date_desc' ? 'date_asc' : 'date_desc')}
                                            className={`px-2.5 py-1.5 rounded-lg text-[11px] font-semibold transition-all border flex items-center gap-1 ${sortMode.startsWith('date_')
                                                ? 'bg-violet-500/20 text-violet-300 border-violet-500/30'
                                                : 'bg-black/30 text-slate-400 border-white/5 hover:bg-white/5'
                                                }`}>
                                        ...
                                        <button onClick={() => setSortMode('workspaces')}
                                            className={`px-2.5 py-1.5 rounded-lg text-[11px] font-semibold transition-all border flex items-center gap-1 ${sortMode === 'workspaces'
                                                ? 'bg-violet-500/20 text-violet-300 border-violet-500/30'
                                                : 'bg-black/30 text-slate-400 border-white/5 hover:bg-white/5'
                                                }`}>
                                ...
                                {/* Reset */}
                                {(planFilter || statusFilter || sortMode !== 'name_asc') && (
                                    <button onClick={() => { setPlanFilter(''); setStatusFilter(''); setSortMode('name_asc'); setPage(1); }}
                                        className="w-full px-3 py-1.5 rounded-lg text-[11px] font-semibold text-slate-400 border border-white/5 hover:bg-white/5 transition-all">
                                        ✕ Reset filtri
                                    </button>
                                )}
                            </div>
