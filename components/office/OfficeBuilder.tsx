...
import {
    ...
    Copy, CheckSquare, ChevronRight, ChevronLeft, Map
} from 'lucide-react';
import { BuilderRoomProperties } from './builder/BuilderRoomProperties';
import { BuilderTemplatesPicker } from './builder/BuilderTemplatesPicker';
import { BuilderConnectionsPanel } from './builder/BuilderConnectionsPanel';
                    ...
                    <div className="flex flex-col overflow-hidden rounded-2xl" style={{ maxHeight: 'inherit' }}>
                        ...
                        {selectedRoom ? (
                            <BuilderRoomProperties
                                ...
                                onAutoArrange={handleAutoArrangeHierarchy}
                            />
                        ) : (
                            <>
                                <div className="p-4 flex-1 flex flex-col justify-start items-center h-full min-h-[160px] relative overflow-y-auto custom-scrollbar">
                                    ...
                                    {!showTemplates ? (<>
                                        ...
                                        {/* ═══ Rami / Connessioni ═══ */}
                                        <BuilderConnectionsPanel
                                            rooms={rooms}
                                            roomConnections={roomConnections}
                                            connectFromId={connectFromId}
                                            setConnectFromId={setConnectFromId}
                                            connectToId={connectToId}
                                            setConnectToId={setConnectToId}
                                            connectColor={connectColor}
                                            setConnectColor={setConnectColor}
                                            connectLabel={connectLabel}
                                            setConnectLabel={setConnectLabel}
                                            onCreateConnection={() => handleCreateConnection()}
                                            onDeleteConnection={handleDeleteConnection}
                                        />
                                    </>) : (
                                        /* Templates List */
                                        <BuilderTemplatesPicker
